  ...
  const handleAddToInitiative = (encounter: SavedEncounter) => {
    for (const em of encounter.monsters) {
      ...
      for (let i = 0; i < em.quantity; i++) {
        const init = rollMode === "auto" ? rollInitiative(sb.initiative_bonus) : sb.initiative_bonus;
        addCombatant({
          name: sb.name,
          initiative: init,
  ...
  return (
    <div className="encounter-panel">
      ...
      {showNew && (
        <EncounterEditor
          obr={obr}
          onSave={(data) => { addEncounter(data); setShowNew(false); }}
          ...
          <div key={enc.id} className="encounter-card">
            ...
            >
              ...
              <div className="encounter-card-actions">
                <button
                  ...
                  onClick={(e) => { e.stopPropagation(); handleAddToInitiative(enc); }}
                >
                  Deploy
                </button>
                <button
                  className="icon-btn"
                  title="Export"
                  onClick={(e) => { e.stopPropagation(); handleExport(enc); }}
                >
                  v
                </button>
                {obr.isGM && (
              ...
              <div className="encounter-detail">
                  ...
                  {enc.monsters.map((em) => {
                    ...
                    return (
                      <div key={em.statBlockId} className="encounter-monster-row">
                        <span className="em-qty">{em.quantity}x</span>
                ...
                {obr.isGM && (
                  <button
                    ...
                    onClick={() => { setEditingId(enc.id); setExpandedId(null); }}
                  >
                    Edit Encounter
                  </button>
                )}
            ...
            {editingId === enc.id && (
              <EncounterEditor
                obr={obr}
                existing={enc}
                onSave={(data) => { updateEncounter(enc.id, data); setEditingId(null); }}
...
function EncounterEditor({ obr, existing, onSave, onCancel }: EncounterEditorProps) {
  ...
  const [monsters, setMonsters] = useState<EncounterMonster[]>(existing?.monsters ?? []);
  ...
  const assignToken = (statBlockId: string, token: OBRToken | null) => {
    setMonsters(monsters.map((m) =>
      m.statBlockId === statBlockId
        ? { ...m, tokenImageUrl: token?.imageUrl, tokenImageId: token?.id }
        : m
    ));
  ...
  return (
    <div className="encounter-editor">
      ...
      <div className="monster-adder">
        <select value={selectedSbId} onChange={(e) => setSelectedSbId(e.target.value)}>
          <option value="">-- Add creature from bestiary --</option>
          {statBlocks.map((sb) => (
            <option key={sb.id} value={sb.id}>{sb.name} (CR {sb.cr})</option>
          ))}
        </select>
        <button className="btn btn-secondary btn-sm" onClick={addMonster} disabled={!selectedSbId}>
          Add
        </button>
      </div>
          ...
          return (
            <div key={em.statBlockId} className="editor-monster-row">
              <div className="em-row-main">
                ...
                <button className="icon-btn danger" onClick={() => updateQuantity(em.statBlockId, 0)}>
                  x
                </button>
              </div>
              ...
              <div className="em-token-row">
                {pickerTokenId === em.statBlockId ? (
                  <div className="token-picker">
                    <button className="token-option none" onClick={() => assignToken(em.statBlockId, null)}>
                      None (no token)
                    </button>
                  ...
                  <button className="token-assign-btn" onClick={() => setPickerTokenId(em.statBlockId)}>
